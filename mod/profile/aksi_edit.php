		...
		if ($foto_type =='image/jpg' OR $foto_type =='image/png' OR $foto_type =='image/jpeg') {
			if ($foto_size < 5024000 ) {
			   ...
			   $queryEdit= mysqli_query($koneksi,"UPDATE profil SET nama_profile='$nama', deskripsi='$deskripsi', foto='$foto' WHERE id_profil='$id'");
				if($queryEdit){
					echo "<script>alert('Data Berhasil diubah '); window.location='$admin_url'+'adminweb.php?mod=profile';</script>";
				}else{
					echo "<script>alert('Gagal mengubah data '); window.location='$admin_url'+'adminweb.php?mod=edit&profil='+'$id';</script>";
				}
			}else{
		   ...
		   echo "<script>alert('Gagal menyimpan data, Tipe foto bukan jpg/png '); window.location='$admin_url'+'adminweb.php?module=add';</script>";
	   }
	}else{
